...
pace$Month <- factor(pace$Month, levels = c("January", "February", "March","April", "May"))
...
# Filter dataset between 250 sec (4:10) and 260 sec (4:20) per kilometer

df_pace <- df %>%
  filter(pace_sec_per_km >= 245, pace_sec_per_km <= 265)

# very few kms in Jan, Feb and March let's filter out these as well and lets only include complete laps i.e. laps =>

df_pace <- df_pace %>% filter((Month == "April" | Month == "May") & Distance >= 1)
```
  ...
  geom_point(
    ...
    scale_color_manual(values = colors) +
      ...
      labs(
        ...
        subtitle = "Distribution of Average HR values for laps recorded at 4:05 - 4:25 min/km. ",
        caption = "Source: <b>Garmin Connect</b> | Graphic: <b>Chris Edwards</b>",
        x = "Month",
